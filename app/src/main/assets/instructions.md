...
Each deck is a separate CSV file. Structure:
...
The app restores `template.csv` and `instructions.md` to their original contents on every launch. If you delete them or modify them by accident, just reopen the app, the files come back. Your own decks are never touched.
